...
> 版本：Draft 1.1
> 日期：2026-08-09
> 适用范围：AI 辅助软件交付、企业级业务系统、项目 Runtime 治理、智能体能力治理、多端业务能力复用。
...
3. 由采用方项目的后端/runtime 管理真相、权限、状态、证据和风险；本仓库提供方法论和门禁。
...
Runtime 是采用方项目中执行标准的底层架构组件。它负责真相、唯一、记忆、学习、状态、权限、证据、风险和演进。本仓库不包含通用 Runtime 实现。
...
| S4 | Runtime 治理 | 项目 Runtime 管理权限、状态、风险、证据和多端调用 |
