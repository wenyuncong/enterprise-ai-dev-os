...
.PARAMETER Scope
    Deploy target scope. "project" deploys into this repository's project
    adapter folders (default). "global" deploys rules/AGENTS.global.md to
    user-level locations (e.g. ~/.codex/AGENTS.md) so every project on this
    machine loads the same methodology kernel; existing files are backed up
    to ~/.enterprise-ai-dev-os/backup/. "both" runs both scopes.
...
function Copy-GeneratedFile {
    param(
        ...
        [string]$Format,
        [string]$ToolName,
        [string]$SourceLabel = "rules/AGENTS.md"
    )
...
"@
    ...
    $normalized = $next.TrimEnd("`r", "`n") + "`n"
    [System.IO.File]::WriteAllText($Destination, $normalized, [System.Text.UTF8Encoding]::new($true))
...
# --- Global (user-level) deploy scope -------------------------------------
if ($Scope -in @("global", "both")) {
    Write-Host ""
    Write-Host "=== Global User-Level Deploy ===" -ForegroundColor Cyan
    $globalRoot = $config.globalTargets
    if (-not $globalRoot) {
        Write-Host "  no globalTargets registered in adapters.json" -ForegroundColor DarkGray
    } else {
        $globalDir = Join-Path $HOME ".enterprise-ai-dev-os"
        $backupDir = Join-Path $globalDir "backup"
        $userPref = Join-Path $globalDir "user-preferences.md"
        $globalResults = @{}

        foreach ($prop in $globalRoot.PSObject.Properties) {
            if ($prop.Name -eq "description") { continue }
            $toolName = $prop.Name
            $g = $prop.Value
            if (-not $g.path) { continue }
            $relPath = $g.path -replace '^~[\\/]', ''
            $dest = Join-Path $HOME $relPath
            $src = Join-Path $ProjectRoot $g.source
            if (-not (Test-Path $src)) {
                Write-Warning "  [$toolName] global source missing: $src"
                $globalResults[$toolName] = $false
                continue
            }
            Write-Host "[$toolName] global rules -> $dest" -ForegroundColor Green
            if ((Test-Path $dest) -and (-not $DryRun)) {
                $stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
                $backupFile = Join-Path $backupDir "$toolName-AGENTS.md.$stamp.bak"
                Ensure-ParentDir $backupFile
                Copy-Item -LiteralPath $dest -Destination $backupFile -Force
                Write-Host "    backup existing -> $backupFile" -ForegroundColor DarkYellow
            }
            $stepOk = Invoke-DeployStep "Global rules: $($g.source) -> $dest" {
                Copy-GeneratedFile -Source $src -Destination $dest -Format $g.format -ToolName $toolName -SourceLabel $g.source
            }
            $globalResults[$toolName] = $stepOk
        }

        if (-not (Test-Path $userPref)) {
            Ensure-ParentDir $userPref
            if ($DryRun) {
                Write-Host "  [DRY] create global memory: $userPref" -ForegroundColor Yellow
            } else {
                $prefTemplate = @"
# 用户偏好与跨项目记忆（全局）
> 由 Enterprise AI Development OS 全局部署创建。跨项目通用偏好、词汇、教训在此累积；项目特定内容留在项目内。

## 已知通用偏好
- 零废话 UI：ERP / SaaS / 管理后台页面只保留可操作提示。
- 编码：UTF-8（推荐 BOM）、LF 换行，.editorconfig 约束。
...
- （暂无）
"@
                [System.IO.File]::WriteAllText($userPref, $prefTemplate.TrimEnd() + "`n", [System.Text.UTF8Encoding]::new($true))
                Write-Host "  created global memory: $userPref" -ForegroundColor DarkGreen
            }
        } else {
            Write-Host "  global memory exists: $userPref" -ForegroundColor DarkGray
        }

        Write-Host "--- Global Summary ---" -ForegroundColor Cyan
        foreach ($name in $globalResults.Keys) {
            $ok = $globalResults[$name]
            $icon = if ($ok) { "[OK]" } else { "[FAIL]" }
            $color = if ($ok) { "Green" } else { "Yellow" }
            Write-Host "  $icon $name (global)" -ForegroundColor $color
        }
    }
}
