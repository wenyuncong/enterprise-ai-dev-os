...
1. Identify the behavior under test and the public API or user interaction that exposes it.
2. Choose the narrowest reliable test type: pure unit, component, integration-style, or snapshot only when appropriate.
3. Mock only external boundaries such as network, storage, timers, or expensive dependencies.
4. Assert observable outcomes, errors, emitted events, DOM state, or returned values.
5. Run the relevant Jest command and fix the first failing assertion or setup error.
...
- Do not test private implementation details when public behavior can be asserted.
- Do not leave mocks shared across tests without reset or cleanup.
- Prefer deterministic tests over sleeps, live network calls, or clock-dependent assertions.
