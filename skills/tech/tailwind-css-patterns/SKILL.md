...
Provide Tailwind CSS composition patterns: utility-first design, responsive breakpoints, dark mode, component extraction, and design token management. Exists to ensure consistent, maintainable Tailwind usage across projects.

## Rule

Tailwind rules: 1) Use utility classes directly in markup, extract to @apply only when repeated 3+ times, 2) Responsive: mobile-first with sm/md/lg/xl breakpoints, 3) Dark mode via class strategy, 4) Custom design tokens in tailwind.config theme.extend, 5) Never use inline styles alongside Tailwind.
