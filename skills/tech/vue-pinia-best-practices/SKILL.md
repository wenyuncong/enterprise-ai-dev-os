...
Pinia rules: 1) One store per domain concept, not per page, 2) Prefer setup stores (composition API style) over options stores, 3) Actions for async operations, never in getters, 4) Store state is reactive, destructure with storeToRefs, 5) Test stores independently of components.
