...
Provide comprehensive Flutter development expertise: widget composition, state management, platform channels, testing strategy, and app architecture. Exists to ensure Flutter apps follow production-grade patterns across all layers.

## Rule

Flutter rules: 1) Widget build methods must be pure and fast, 2) State management via Provider/Riverpod/Bloc (pick one, be consistent), 3) Platform-specific code behind abstraction, 4) Test at widget level, not just unit, 5) const constructors everywhere possible.
