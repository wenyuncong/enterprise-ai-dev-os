...
Provide Vue 3 production best practices: project structure, state management, performance optimization, testing strategy, and deployment patterns. Exists to ensure Vue applications are built for production with proven patterns.

## Rule

Vue best practices: 1) Pinia for state management (not Vuex), 2) Lazy-load routes for code splitting, 3) v-memo for expensive lists, 4) Component testing with Vue Test Utils + jsdom, 5) E2E with Playwright or Cypress.
