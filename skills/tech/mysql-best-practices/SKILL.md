...
MySQL rules: 1) Every table has a primary key, 2) Index foreign keys and WHERE clause columns, 3) Use EXPLAIN before deploying queries, 4) Transactions for multi-statement mutations, 5) Migrations versioned and reversible, 6) No SELECT * in production code.

# MySQL Best Practices

## Purpose

Use this skill to keep MySQL schema and query work correct, durable, performant, and recoverable.

## Workflow

1. Verify the current table structure, indexes, row counts, and constraints before changing SQL.
2. Choose data types, keys, indexes, and transaction boundaries from the access pattern.
3. Test important queries with `EXPLAIN` or equivalent evidence.
4. Keep migration, rollback, and seed data separate.
5. Record schema-impact evidence for review.
