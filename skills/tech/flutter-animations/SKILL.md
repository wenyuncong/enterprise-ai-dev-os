...
Provide Flutter animation patterns and best practices: implicit/explicit animations, custom painters, hero transitions, staggered animations, and performance optimization. Exists to ensure Flutter animations are smooth, performant, and maintainable.

## Rule

Animation rules: 1) Prefer implicit animations (AnimatedContainer, AnimatedOpacity) for simple cases, 2) Use AnimationController for complex sequences, 3) Always dispose controllers, 4) Test on target device, not just emulator, 5) Keep animation durations under 300ms for UI feedback.
